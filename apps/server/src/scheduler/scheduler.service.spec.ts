...
describe("SchedulerService (unit)", () => {
  ...
  let mockNotificationQueue: { enqueueNotification: ReturnType<typeof vi.fn> };
  let mockNotificationConfig: { isConfigured: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    service = new SchedulerService(
      ...
      mockNotificationQueue as unknown as NotificationQueueWorker,
      mockNotificationConfig as unknown as NotificationConfigService,
    );
  });
  ...
  describe("notification enqueue", () => {
    it("does not enqueue when notifyEmail is false", async () => {
      ...
      mockPrisma.watch.findUniqueOrThrow.mockResolvedValue(watch);
      mockNotificationConfig.isConfigured.mockReturnValue(true);

      await (service as unknown as { executeAndRecord(id: string): Promise<unknown> })
        .executeAndRecord("watch-1");

      expect(mockNotificationQueue.enqueueNotification).not.toHaveBeenCalled();
    });

    it("does not enqueue when notifications are not configured", async () => {
      const watch = makeWatch({ notifyEmail: true });
      mockPrisma.watch.findUniqueOrThrow.mockResolvedValue(watch);
      mockNotificationConfig.isConfigured.mockReturnValue(false);
    ...
    it("enqueues CONDITION_MET when notifyEmail=true, configured, and edge detected (no prior run)", async () => {
      const watch = makeWatch({ notifyEmail: true });
      mockPrisma.watch.findUniqueOrThrow.mockResolvedValue(watch);
      mockNotificationConfig.isConfigured.mockReturnValue(true);
      ...
      expect(mockNotificationQueue.enqueueNotification).toHaveBeenCalledWith("watch-1", "run-1", "CONDITION_MET");
    });

    it("enqueues EXECUTION_ERROR when notifyEmail=true, configured, and first ERROR run", async () => {
      const watch = makeWatch({ notifyEmail: true });
      mockPrisma.watch.findUniqueOrThrow.mockResolvedValue(watch);
      mockNotificationConfig.isConfigured.mockReturnValue(true);
      mockPrisma.watchRun.findFirst.mockResolvedValue(null);
      ...
      expect(mockNotificationQueue.enqueueNotification).toHaveBeenCalledWith("watch-1", "run-2", "EXECUTION_ERROR");
    });
